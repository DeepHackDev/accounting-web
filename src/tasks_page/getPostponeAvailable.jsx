...
const getPostponeAvailable = task => {
  if (task.postponedDate) {
    ...
    if (postponedDatediff <= task.period.amount) return false;
  }
  ...
  if (task.executionDates.length>0) {
    ...
    if (datediff < task.period.amount) return false;
    if (datediff === task.period.amount) return true;
  }
  ...
  if (createdAtDatediff <= task.period.amount) return true;

  return false;
}

export { getPostponeAvailable };
